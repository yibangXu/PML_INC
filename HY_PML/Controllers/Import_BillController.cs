...
VALUES(@DC, @SUBNO, @SUBST, @IDNO, @MONEYS, @dPTNO, @TransferBy, @TransferTime, @CcNo, @DestNo, @PType, @PiecesNo, @Weight, @Freight, @CustomsPay, @Tariff, @OtherPayTax, @OtherPayNoTax, @ToPayment, @ToPaymentCurrency, @AgentPay, @AgentPayCurrency,@LadingDate,@AStatNo,@SendCHName,@CName,@StatNo,@SectorNo,@SectorName,@HubName,@SendCustAddr,@SendECustAddr,@AStatName,@StatName,@HubNo,@SendCustNo,@SStatName,@RecCompany,@SendBy,@SendEBy,@RecBy,@Volume); 
IF @@Error <> 0 BEGIN SET @chk = 1 END");
