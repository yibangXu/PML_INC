...
{
	public class Bill_Lading_IsReviewController : Controller
	{
		...
		public ActionResult Index()
		{
			...
			ViewBag.Title = "提單財務審核";
			ViewBag.dlgWidth = "960px";
			ViewBag.ControllerName = "Bill_Lading_IsReview";
			ViewBag.AddFunc = "";
			ViewBag.EditFunc = "";
			ViewBag.DelFunc = "";
			ViewBag.SaveRecFunc = "";
		...
		public ActionResult GetGridJSON(Bill_Lading data, int page = 1, int rows = 40, DateTime? start_date = null, DateTime? end_date = null)
		{
			...
			var billLading =
				...
				where statNoList.Contains(b.SStatNo) && b.IsCheck == true && b.IsConfirm == true && b.IsDelete == false
				select new
				{
					LadingNo = b.LadingNo,
			...
			if (data.ImOrEx.IsNotEmpty() || data.ImOrEx != null)
				billLading = billLading.Where(x => x.ImOrEx == data.ImOrEx);
			if (data.IsReview.HasValue)
				billLading = billLading.Where(x => x.IsReview == data.IsReview);
		...
		[Authorize]
		public ActionResult BillLadingReview(string[] LadingNo)
		{
			var result = new ResultHelper();
			var repeat = false;
			using (var trans = db.Database.BeginTransaction())
			{
				var ExData = new List<Export_Bill>();
				var ImData = new List<Import_Bill>();
				foreach (var l in LadingNo)
				{
					var data = db.Bill_Lading.Where(x => x.LadingNo == l && x.IsDelete == false && x.IsConfirm == true && x.IsCheck == true).FirstOrDefault();
					if (data.IsReview == true)
						repeat = true;
					data.IsReview = true;
					data.ReviewTime = DateTime.Now;
					data.ReviewBy = User.Identity.Name;
					...
					if (data.ImOrEx == "Ex")
					{
						ExData.Add(new Export_Bill()
						{
							ExBillNo = "EX-" + data.LadingNo,
							LadingNo = data.LadingNo,
							LadingDate = data.LadingDate,
							StatNo = null,
							StatName = null,
							CustNo = data.SendCustNo,
							CHName = data.SendCHName,
							CcNo = data.CcNo,
							DestNo = data.DestNo,
							CName = data.CName,
							Type = data.Type == "0" ? "DOX" : data.Type == "1" || data.Type == "2" ? "SPX" : data.Type,
							PiecesNo = data.PiecesNo ?? 0,
							Weight = data.Weight,
							Freight = data.Freight,
							CustomsPay = data.CustomsPay,
							Tariff = 0,
							ProdIdPay = data.ProdIdPay,
							InsurancePay = data.InsurancePay,
							OtherPayTax = data.OtherPayTax,
							OtherPayNoTax = data.OtherPayNoTax,
							ToPayment = data.ToPayment,
							ToPaymentCurrency = data.ToPaymentCurrency,
							AgentPay = data.AgentPay,
							AgentPayCurrency = data.AgentPayCurrency,
							Remark = data.Remark,
							Total = data.Freight + data.CustomsPay + data.ProdIdPay + data.InsurancePay + data.OtherPayTax + data.OtherPayNoTax + data.ToPayment + data.AgentPay,
					...
					{
						ImData.Add(new Import_Bill()
						{
							ImBillNo = "Im-" + data.LadingNo,
							LadingNo = data.LadingNo,
							LadingDate = data.LadingDate,
							StatNo = null,
							StatName = null,
							...
							AgentPayCurrency = data.AgentPayCurrency,
							Remark = data.Remark,
							Total = data.Freight + data.CustomsPay + data.OtherPayTax + data.OtherPayNoTax + data.ToPayment + data.AgentPay,
				...
				if (repeat == true)
				{
					...
					result.Message = "選擇資料中，已存在審核過之資料";
					trans.Rollback();
				}
				else
				{
					if (ExData.Count != 0)
						db.Export_Bill.AddRange(ExData);
					if (ImData.Count != 0)
						db.Import_Bill.AddRange(ImData);
