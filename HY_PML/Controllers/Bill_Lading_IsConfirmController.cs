...
{
	public class Bill_Lading_IsConfirmController : Controller
	{
		...
		public ActionResult Index()
		{
			...
			ViewBag.ControllerName = "Bill_Lading_IsConfirm";
			ViewBag.AddFunc = "";
			ViewBag.EditFunc = "";
			ViewBag.DelFunc = "";
			ViewBag.SaveRecFunc = "";
		...
		public ActionResult GetGridJSON(Bill_Lading data, int page = 1, int rows = 40, DateTime? start_date = null, DateTime? end_date = null)
		{
			...
			if (data.IsConfirm.HasValue)
				billLading = billLading.Where(x => x.IsConfirm == data.IsConfirm);
			...
			billLading = billLading.OrderByDescending(o => o.CreateTime).Skip((page - 1) * rows).Take(rows);
		...
		[Authorize]
		public ActionResult BillLadingConfirm(string[] LadingNo)
		{
			var result = new ResultHelper();
			var repeat = false;
			using (var trans = db.Database.BeginTransaction())
			{
				...
				{
					var data = db.Bill_Lading.Where(x => x.LadingNo == l && x.IsDelete == false).FirstOrDefault();
					if (data.IsConfirm == true)
						repeat = true;
					data.IsConfirm = true;
					data.ConfirmBy = User.Identity.Name;
				...
				if (repeat == true)
				{
					...
					result.Message = "選擇資料中，已存在確認過之資料";
					trans.Rollback();
				}
				else
				{
